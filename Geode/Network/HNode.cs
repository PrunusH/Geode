...
using Geode.Cryptography.Ciphers;
...
namespace Geode.Network
{
    public class HNode : IDisposable
    {
        ...
        //Ported methods from old version
        public Task<HPacket> ReceivePacketAsync()
        {
            return HFormat.EvaWire.ReceivePacketAsync(this).AsTask();
        }
        public Task<int> SendPacketAsync(HPacket packet)
        {
            return SendAsync(packet.ToBytes()).AsTask();
        }
        public Task<int> SendPacketAsync(string signature)
        {
            return SendAsync(EvaWirePacket.ToBytes(signature)).AsTask();
        }
        public Task<int> SendPacketAsync(ushort id, params object[] values)
        {
            return SendAsync(HFormat.EvaWire.Construct(id, values)).AsTask();
        }
        //
